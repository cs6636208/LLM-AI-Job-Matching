...
const canAccessJob = (user, job) =>
  user.role === "ADMIN" || job.userId === user.id;
...
router.get("/", requireAuth, async (req, res) => {
  try {
    ...
    const jobs = await prisma.job.findMany({
      ...
      include: {
        _count: {
          select: { jobCandidates: true, rubrics: true, interviews: true },
        },
      },
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (existing)
        return res.status(400).json({ error: "Candidate already in this job" });
      ...
      await logActivity(
        ...
        {
          jobId,
          jobTitle: (await prisma.job.findUnique({ where: { id: jobId } }))
            ?.title,
        },
      );
